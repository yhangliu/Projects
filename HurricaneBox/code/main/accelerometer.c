...
int16_t read16(uint8_t reg1, uint8_t reg2) {
  u_int8_t data1 = readRegister(reg1);
  u_int8_t data2 = readRegister(reg2);
  u_int16_t data = 0x0000;

  data = ((u_int16_t)data2 << 8) | data1;
  return data;
}
...
void getAccel(float * xp, float *yp, float *zp) {
  *xp = read16(ADXL343_REG_DATAX0,ADXL343_REG_DATAX1) * ADXL343_MG2G_MULTIPLIER * SENSORS_GRAVITY_STANDARD;
  *yp = read16(ADXL343_REG_DATAY0,ADXL343_REG_DATAY1) * ADXL343_MG2G_MULTIPLIER * SENSORS_GRAVITY_STANDARD;
  *zp = read16(ADXL343_REG_DATAZ0,ADXL343_REG_DATAZ1) * ADXL343_MG2G_MULTIPLIER * SENSORS_GRAVITY_STANDARD;
 printf("X: %.2f \t Y: %.2f \t Z: %.2f\n", *xp, *yp, *zp);
 accelerometer_values[0] = *xp;
 accelerometer_values[1] = *yp;
 accelerometer_values[2] = *zp;
}

// function to print roll and pitch
void calcRP(float x, float y, float z){
  // YOUR CODE HERE
  double tmp_roll = atan2(y, z) * 57.3;
  double tmp_pitch = atan2((-x), sqrt(y*y+z*z)) * 57.3;
